...
run_doc_dir = '0719_gs66504b_trap_600v_2mhz/' # the name of your run folder
dut_dir = 'DUT_runs/' # shouldn't need to change
operating_point = 'trap_dvdt_0.3' # the point of interest in sweep. no txt or csv extension
ch1_inverted = True # since I accidentally ran a few with ch1 inverted
...
if use_custom_zeroing_windows:
    ...
    # vref
    z_window_vref_left = 0.3 # vref to the left
    z_window_vref_right = 0.7 # vref to the right
else:
...
show_integration_window_plot = True # whether to show plot for tuning window
use_custom_integration_windows = True # whether to use custom integration window
if use_custom_integration_windows:
    int_window_point_shift_1 = -3 # number of points to the right to shift start of "up integration window"
    int_window_point_shift_2 = -2 # number of points to the right to shift end of "up integration window"
    int_window_point_shift_3 = -3 # number of points to the right to shift start of "down integration window"
    int_window_point_shift_4 = 0 # number of points to the right to shift end of "down integration window"
else:
...
vout_plus_custom_deskew = -1.5 * 1e-9 # [seconds] to right
vout_minus_custom_deskew = 1.17 * 1e-9 # [seconds] to right

# plots to enable or disable
plot_input_traces = False # shows plot of scope data after cdiv scaling has happened
plot_trap_dvdt = False # shows plot of DUT voltage with calculated dv/dt over integration window
...
if ch1_inverted: # for the case where I accidentally ran ch1 inverted
    scope_data[:,1] = -1 * scope_data[:,1] # just flip it back
...
if plot_trap_dvdt:
    ...
    vpp1 = np.average(vdut[istop1+5:istop1+buffer_points]) # average after rising edge
    vpp2 = np.average(vdut[istart2-buffer_points:istart2-5]) # average before falling edge
    vpp_dut = np.average([vpp1, vpp2])
    ax_dvdt[0].plot(np.array([1, 1])*t[istop1+5]*1e9, 
                    np.array([min(vdut), max(vdut)]), '--',
                    color = su_color_dict.get('palo_alto') + '77')
    ax_dvdt[1].plot(np.array([1, 1])*t[istart2-5]*1e9, 
                    np.array([min(vdut), max(vdut)]), '--',
                    color = su_color_dict.get('palo_alto') + '77')
    ax_dvdt[0].plot(np.array([t[istart1-buffer_points], t[istop1+buffer_points]])*1e9, 
                    np.array([1, 1]) * vpp_dut, '--',
                    color = su_color_dict.get('palo_alto') + '77')
    ax_dvdt[1].plot(np.array([t[istart2-buffer_points], t[istop2+buffer_points]])*1e9, 
                    np.array([1, 1]) * vpp_dut, '--',
                    color = su_color_dict.get('palo_alto') + '77')
